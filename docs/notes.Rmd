...
-   This is essentially occurring when we use `package::function()`, but the standard practice assumes we've imported all functions within the package using `library(package)`
...
-   Again, we can see this when we explicitly call `package::function()`, but `box` uses **explicit** function and package **namespacing**
